...
function OpenPlayer(params) {
    ...
    var myWindow;
    myWindow = window.open('http://hypster.com/hypsterPlayer/MPL?' + params, 'HypsterPlayer', 'location=0, status=0, scrollbars=0, width=' + player_W + ', height=' + player_H);
    myWindow.focus();
}
...
function StartMusicSearch() {
    ...
    if (ss_str != " Enter Artist, Song or Genre") {
        ...
        ss_str = ss_str.replace("?", "_");
        ss_str = ss_str.replace("!", "_");
        window.location = "/listen/" + ss_str + "?t=q";
    }
}
...
function SearchMusicStrPage(search_string, page) {
    ...
    $.ajax({
        type: "POST",
        url: "/search/Music?ss=" + ss + "&page=" + page,
        async: true,
        success: function (data) {
            $('#listenSlideContHolder').html(data);
        }
